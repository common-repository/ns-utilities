...
class phpthumb {
    ...
    var $phpthumb_version = '1.7.8-200708080803';
                    ...
                    switch ($command) {
                        ...
                        case 'edge':
                            if ($this->ImageMagickSwitchAvailable('edge')) {
                                $commandline .= ' -edge '.($parameter ? intval($parameter) : 1);
                                unset($this->fltr[$filterkey]);
                            }
                            break;
                        ...
                        case 'wb':
                            if ($this->ImageMagickSwitchAvailable(array('channel', 'contrast-stretch'))) {
                                ...
                                $threshold = ($threshold ? min(max(floatval($threshold), 0), 100) : 0.1);
                        ...
                        case 'blur':
                            if ($this->ImageMagickSwitchAvailable('blur')) {
                                ...
                                $radius = ($radius ? min(max(intval($radius), 0), 25) : 1);
            ...
            foreach ($this->fltr as $filtercommand) {
                ...
                switch ($command) {
                    ...
                    case 'wmi': // WaterMarkImage
                        @list($filename, $alignment, $opacity, $margin['x'], $margin['y']) = explode('|', $parameter, 5);
                        $alignment   = ($alignment           ? $alignment   : 'BR');
